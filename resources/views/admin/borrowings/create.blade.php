            ...
            <form action="{{ route('admin.borrowings.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control @error('status') is-invalid @enderror"
                        required>
                        <option value="peminjaman" {{ old('status') == 'peminjaman' ? 'selected' : '' }}>Peminjaman
                        </option>
                        <option value="pengembalian" {{ old('status') == 'pengembalian' ? 'selected' : '' }}>Pengembalian
                        </option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="denda" id="denda" step="0.01"
                        class="form-control @error('denda') is-invalid @enderror" value="{{ old('denda') }}">
                    @error('denda')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
